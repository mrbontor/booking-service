...
module.exports = {
    signIn: async (payload) => {
        ...
        if (!user.isActive) {
            ...
            throw new UnprocessableEntityError('Your account has been deactive, please contact your administrator!');
        }
    ...
    refreshToken: async (payload) => {
        try {
            ...
            const user = await UserRepository.getByUserEmail(decryptedPayload.email);
            if (!user) {
            ...
            if (err.message === 'jwt expired') {
                ...
                //remove all token
                await UserRepository.updateCustom({ email: decodeExpiredToken.email }, { $unset: { token: 1 } });
    ...
    signOut: async (payload) => {
        ...
        await UserRepository.updateCustom({ email: decryptedPayload.email }, removeToken);
